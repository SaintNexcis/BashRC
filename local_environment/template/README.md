...
2. (Optional) Start from the public template:
   Copy the example files from `local_environment/template/` as a starting point.

3. Add your sensitive files using clean, generic names (no project name in filename for extra obscurity):
   Recommended:
...
5. On the target machine(s), install the overrides:
   ```bash
